...
export async function getPrice({ chain = 'bsc', buyTokenAddress, sellTokenAddress, sellTokenAmount, buyTokenAmount, buyTokenFee, ownerAddress, feeOwner, slippage, gasPrice }) {
  let url = `https://api.swapper.com/v1/swap/${chain.toLowerCase()}/price?buyTokenAddress=${buyTokenAddress}&sellTokenAddress=${sellTokenAddress}&apiKey=${this.config.apiKey}`;
...
export async function getQuote({ chain = 'bsc', buyTokenAddress, sellTokenAddress, sellTokenAmount, buyTokenAmount, buyTokenFee, ownerAddress, feeOwner, slippage, gasPrice }) {
  let url = `https://api.swapper.com/v1/swap/${chain.toLowerCase()}/quote?buyTokenAddress=${buyTokenAddress}&sellTokenAddress=${sellTokenAddress}&apiKey=${this.config.apiKey}`;
...
export async function sendTransaction({ chain = 'bsc', ownerAddress, buyTokenAddress, sellTokenAddress, sellTokenAmount, allowanceAddress }) {
  ...
  let res = null;
  try {
    res = await axios.get(`https://api.swapper.com/v1/swap/${chain.toLowerCase()}/quote?buyTokenAddress=${buyTokenAddress}&sellTokenAddress=${sellTokenAddress}&sellTokenAmount=${sellTokenAmount}&takerAddress=${ownerAddress}`);
  } catch (err) {
